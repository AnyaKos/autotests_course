...
def everything_for_your_cat(cats_data):
    """
    Оптимизация хранения данных, чтобы для каждого владельца, данные о всех его животных отображались в одной строке
    :param cats_data: Список кортежей со всеми записями о котах и покупателях
    :return: (our_str) Данные для каждого владельца о всех его животных отображаются в одной строке
    """
    our_str = ""
    cats_data_dict = {}
    for j in cats_data:
        buyer_name = j[2] + " " + j[3]
        cats_name = j[0] + ", " + str(j[1])
        if buyer_name in cats_data_dict.keys():
            cats_name_old = cats_data_dict.get(buyer_name)
            cats_data_dict.update({buyer_name: cats_name_old + "; " + cats_name})
        else:
            cats_data_dict.update({buyer_name: cats_name})
    for key in cats_data_dict:
        our_str = our_str + f"{key}: {cats_data_dict.get(key)}\n"
    return our_str
