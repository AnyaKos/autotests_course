...
def repeats(our_str):
    """
    Считает сколько раз каждый символ уже встречался, и добавляет количество к символам с помощью постфикса формата _n.
    :param our_str: Входная строка
    :return: (new_str) Новая строка с указанием количества символ встретившихся в строке с помощью постфикса формата _n
    """
    new_str = ""
    letters_dict = {}
    for j in our_str:
        if j in letters_dict.keys():
            ...
            new_str = new_str + str(j) + "_" + str(letters_dict.get(j))
        else:
