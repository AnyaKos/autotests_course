...
class Segment:
    """Обработка координат отрезка"""
    def __init__(self, coordinates_point_1: tuple, coordinates_point_2: tuple):
        """
        Инициализация класса Segment
        :param coordinates_point_1: Кортеж координат первой точки
        :param coordinates_point_2: Кортеж координат второй точки
        """
        self.x1 = coordinates_point_1[0]
        self.y1 = coordinates_point_1[1]
        self.x2 = coordinates_point_2[0]
        self.y2 = coordinates_point_2[1]

    def length(self):
        """
        Возвращает длину отрезка, с округлением до 2 знаков после запятой
        :return: (length_segment) Длина отрезка с округлением до 2 знаков после запятой
        """
        length_segment = ((self.x2-self.x1)**2+(self.y2-self.y1)**2)**(1/2)
        return round(length_segment, 2)

    def x_axis_intersection(self):
        """
        Возвращает True, если отрезок пересекает ось абцисс, иначе False
        :return: Пересечение оси абцисс True иначе False
        """
    ...
    def y_axis_intersection(self):
        ...
        :return: Пересечение оси ординат True иначе False
        """
