...
def move_zeros(lst):
    """
    Переместить все нули в конец, сохраняя порядок остальных элементов
    :param lst: Исходный список
    :return: (lst) Измененный список с нулями, перемещенными в конец списка
    """
    count_nules = lst.count(0)
    while 0 in lst:
        lst.remove(0)
    [lst.append(0) for k in range(count_nules)]
    return lst
